    ...
    <div>
                ...
                >
                    <q-card
                        ...
                        <q-card-section class="text-center">
                            ...
                            <div class="text-caption text-capitalize">{{ v.code }}</div>
                        </q-card-section>
                        <q-card-section class="text-center">
                            <div class="text-caption text-uppercase">api key</div>
                            <div class="text-caption text-grey">
                                {{ LimitCharacter(v.key) }}
                            </div>
                        </q-card-section>
                        <div class="card-overlay absolute-full flex flex-center text-white">
                            <div class="q-gutter-xs">
                                ...
                                <q-btn v-if="v.isActive" unelevated size="xs" color="primary" label="copy api key" @click="Copy"/>
                            </div>
                        </div>
                        <div
        ...
        <q-footer class="bg-white no-shadow q-mx-lg q-mb-md q-py-sm radius-xs text-grey">
            <q-toolbar>
                ...
                <q-input outlined dense debounce="1000" v-model="filter" placeholder="Search..." @update:model-value="LoadAllSystems">
        ...
        <q-dialog persistent v-model="modal" full-height>
            <q-card class="column full-height" style="width: 75%; max-width: 100vw;">
                <q-card-section class="row items-center q-pa-lg">
                    <div class="text-h5 text-uppercase">{{ isEdit ? 'modify system' : 'create new system' }}</div>
                </q-card-section>
                <q-separator inset/>
                <q-card-section class="col scroll">
                    <div class="row q-col-gutter-xs">
                        <div class="col-2">
                            <div class="text-caption text-uppercase" :class="formErrors.code.type ? 'text-negative' : 'text-grey'">{{ formErrors.code.type ? formErrors.code.msg : 'system alias' }}</div>
                            <q-input 
                                v-model="code"
                                label="Enter System Alias"
                                outlined 
                        ...
                        <div class="col-3">
                            <div class="text-caption text-uppercase" :class="formErrors.name.type ? 'text-negative' : 'text-grey'">{{ formErrors.name.type ? formErrors.name.msg : 'system name' }}</div>
                            <q-input 
                                v-model="name"
                        ...
                        <div v-if="isEdit" class="col-3">
                            <div class="text-caption text-uppercase text-grey">api key</div>
                            <q-input 
                                v-model="key"
                                outlined 
                                readonly
                            >
                                <template v-slot:append>
                                    <q-btn round flat unelevated size="sm" icon="content_copy" @click="Copy"/>
                                </template>
                            </q-input>
                        </div>
                    </div>
...
const formValidations = () => {
    ...
    if (!name.value) {
        ...
        formErrors.name.msg = 'system name is required';
        isError = true;
    } else {
    ...
    if (!code.value) {
        ...
        formErrors.code.msg = 'system alias is required';
        isError = true;
    } else {
...
const Create = async () => {
    ...
    try {
        ...
        systems.value.push(response.data.Data);
    } catch (e) {
        errors.value = [];
...
const Update = async () => {
    ...
    innerLoading.value = true;
    try {
        const response = await api.post(`/system/${id.value}/update`, {
            name: name.value,
            code: code.value
        });
        ResetForm();
...
const UpdateList = async (data) => {
    const index = systems.value.findIndex(item => item.id === data.id)
    if (index !== -1) {
        systems.value[index] = data
    }
}
...
const Disable = async (data) => {
    ...
    try {
        const response = await api.post(`/system/${data.id}/disable`);
        Toast.fire({
            icon: "success",
            html: `
                <div class="text-h6 text-bold text-uppercase">granted!</div>
...
const Enable = async (data) => {
    btnLoading.value = true;
    try {
        const response = await api.post(`/system/${data.id}/enable`);
        Toast.fire({
            icon: "success",
            html: `
                <div class="text-h6 text-bold text-uppercase">granted!</div>
...
const Copy = async () => {
    try {
        ...
        Toast.fire({
            ...
            html: `
                <div class="text-h6 text-bold text-uppercase">granted!</div>
                <div class="text-caption; text-capitalize;">API Key copied to clipboard<div>
            `
        });
    } catch (e) {
        alert('not')
