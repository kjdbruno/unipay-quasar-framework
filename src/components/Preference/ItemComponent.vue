    ...
    <div>
            ...
            <q-card-section>
                <div class="row items-center no-wrap justify-between">
                    <div class="row items-center">
                        ...
                        <span class="text-h5 text-dark text-uppercase q-ml-sm">item management</span>
                    </div>
                    <div class="col-auto">
                        <q-input outlined dense debounce="300" v-model="filter" placeholder="Search...">
                            <template v-slot:before>
                                <q-btn :disable="page <= 1" @click="PreviousPage" unelevated size="xs" round color="primary" icon="arrow_back"/>
                                <q-btn v-if="items.length" unelevated outline size="sm" round color="primary" :label="meta.CurrentPage"/>
            ...
            <transition-group name="fade-slide" tag="div" class="q-gutter-md row q-col-gutter-md">
                <q-card v-for="(v, index) in filteredData" :key="index" class="card card-hover-animate custom-border col-xs-12 col-sm-4 col-md-3 col-lg-3 flex flex-center q-pa-md no-shadow cursor-pointer radius-xs">
                    <q-card-section class="text-center">
                        ...
                        <div class="text-caption text-capitalize text-grey">{{ v.RevenueService?.Office?.name }}</div>
                    </q-card-section>
                    <q-card-section class="text-center">
                        <div class="text-caption text-grey">{{ v.isManual ? 'Manually' : v.cost }}</div>
                    </q-card-section>
        ...
        <q-page-sticky position="bottom-right" :offset="[18, 18]">
            <q-spinner-oval
                ...
                size="3em"
            />
            <q-card class="no-shadow custom-border" v-if="!loading && !items.length">
            ...
            <q-card class="card-action column full-height">
                ...
                <q-card-section class="col">
                    ...
                    <q-select 
                        ...
                        :error="formErrors.serviceId.type"
                        :hide-dropdown-icon="true"
                        class="q-mb-xs"
                    >
                        <template v-if="serviceId" v-slot:append>
                            <q-icon name="delete" size="xs" @click.stop.prevent="serviceId = null" class="cursor-pointer" />
                        </template>
                    </q-select>
                    <q-select 
                        v-model="receiptId" 
                        emit-value 
                        map-options 
                        :options="filteredReceipts" 
                        label="Search Receipt" 
                        outlined
                        use-input 
                        input-debounce="300" 
                        @filter="filterReceiptFn" 
                        :error="formErrors.receiptId.type"
                        :hide-dropdown-icon="true"
                        class="q-mb-xs"
                    >
                        <template v-if="receiptId" v-slot:append>
                            <q-icon name="delete" size="xs" @click.stop.prevent="receiptId = null" class="cursor-pointer" />
                        </template>
                    </q-select>
                    <div class="q-mb-xs">
                        <q-checkbox
...
const formValidations = () => {
    ...
    if (!serviceId.value) {
        ...
        formErrors.serviceId.type = true;
        ...
        formErrors.serviceId.type = null

    }

    if (!receiptId.value) {

        formErrors.receiptId.type = true;
        ...
        formErrors.receiptId.type = null
    ...
    if (!isManual.value) {
        ...
        if (!cost.value) {
            ...
            errors.value.push({
                type: 'field',
                value: cost.value,
                msg: 'Cost must be a valid number (decimals allowed)',
                path: 'cost',
                location: 'body'
            });
...
const Create = async () => {
    ...
    try {
        ...
        (items.value.length ? (items.value.push(response.data.Data)) : LoadAll());
    } catch (e) {
...
const Modify = async (data) => {
    ...
    filteredReceipts.value = receipts.value
}
    ...
    return (val, update) => {
        update(() => {
            if (!val) {
                ...
                targetRef.value = sourceRef.value.filter(v =>
...
const filterReceiptFn = createFilterFn(receipts, filteredReceipts)

const LoadServices = async () => {
    try {
        const response = await api.get('/option/service');
...
const LoadReceipts = async () => {
    try {
        const response = await api.get('/option/receipt');
