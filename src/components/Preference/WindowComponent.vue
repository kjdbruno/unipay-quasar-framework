            ...
            <q-card-section>
                <div class="row items-center no-wrap justify-between">
                    <div class="row items-center">
                        ...
                        <span class="text-h5 text-dark text-uppercase q-ml-sm">window management</span>
                    </div>
                    <div class="col-auto">
        ...
        <q-dialog square persistent v-model="modal" position="right" full-height class="dialog-action">
            <q-card class="card-action column full-height">
                <q-card-section class="row items-center q-pa-lg">
                    <div class="text-h5 text-uppercase">{{ isEdit ? 'modify window' : 'create new window' }}</div>
                <q-space />
...
const LoadAll = async () => {
    loading.value = true;
    try {
        const { data } = await api.get('/window', {
            params: { 
                Page: page.value, 
                Limit: limit.value 
            }
        ...
        console.error("Error fetching all windows:", error);
    } finally {
        loading.value = false
    }
}
...
const Create = async () => {
    ...
    try {
        const response = await api.post(`/window`, {
            name: name.value,
        ...
        windows.value.push(response.data.Data);
    } catch (e) {
...
const Update = async () => {
    ...
    innerLoading.value = true;
    try {
        const response = await api.post(`/window/${id.value}/update`, {
            name: name.value,
            description: description.value
        });
        ResetForm();
...
const UpdateList = async (data) => {
    const index = windows.value.findIndex(item => item.id === data.id)
    if (index !== -1) {
        windows.value[index] = data
    }
}
...
const Disable = async (data) => {
    ...
    try {
        const response = await api.post(`/window/${data.id}/disable`);
        Toast.fire({
            icon: "success",
            html: `
                <div class="text-h6 text-bold text-uppercase">granted!</div>
...
const Enable = async (data) => {
    btnLoading.value = true;
    try {
        const response = await api.post(`/window/${data.id}/enable`);
        Toast.fire({
            icon: "success",
            html: `
                <div class="text-h6 text-bold text-uppercase">granted!</div>
