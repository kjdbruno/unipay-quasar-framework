    ...
    <div>
        <div class="q-mt-md">
            <div class="card-grid">
                ...
                <div
                    v-if="!loading && !paymentchannels.length"
                    class="card-anim-wrapper"
                    :style="{ animationDelay: `120ms` }"
                >
                    <q-card
                ...
                <div
                    v-if="!loading && paymentchannels.length"
                    v-for="(data, index) in paymentchannels"
                    :key="`data-${data.id}`"
                    class="card-anim-wrapper"
        ...
        <q-dialog persistent v-model="modal" full-height>
            <q-card class="column full-height" style="width: 75%; max-width: 100vw;">
                <q-card-section class="row items-center q-pa-lg">
                    <div class="text-h5 text-uppercase">{{ isEdit ? 'modify payment channel' : 'create new payment channel' }}</div>
                </q-card-section>
                <q-separator inset/>
                <q-card-section class="col scroll">
                    <div class="row q-col-gutter-xs">
                        <div class="col-3">
                            <div class="text-caption text-uppercase" :class="formErrors.name.type ? 'text-negative' : 'text-grey'">{{ formErrors.name.type ? formErrors.name.msg : 'channel name' }}</div>
                            <q-input 
                                v-model="name"
                                label="Enter Payment Channel Name"
                                outlined 
...
const formValidations = () => {
    ...
    if (!name.value) {
        ...
        formErrors.name.msg = 'channel name is required';
        isError = true
    } else {
...
const LoadAll = async () => {
    loading.value = true;
    try {
        const { data } = await api.get('/paymentchannel', {
            params: { 
                Page: page.value, 
                Limit: limit.value,
                Filter: filter.value
...
const filteredData = computed(() => {
    ...
    let data = paymentchannels.value.slice()
...
const Create = async () => {
    ...
    try {
        const response = await api.post(`/paymentchannel`, {
            name: name.value
        });
        ResetForm();
        modal.value = false;
        ...
        (paymentchannels.value.length ? (paymentchannels.value.push(response.data.Data)) : LoadAll());
    } catch (e) {
...
const Update = async () => {
    ...
    innerLoading.value = true;
    try {
        const response = await api.post(`/paymentchannel/${id.value}/update`, {
            name: name.value
        });
        ResetForm();
        modal.value = false;
...
const UpdateList = async (data) => {
    const index = paymentchannels.value.findIndex(item => item.id === data.id)
    if (index !== -1) {
        paymentchannels.value[index] = data
    }
}
...
const Disable = async (data) => {
    ...
    try {
        const response = await api.post(`/paymentchannel/${data.id}/disable`);
        Toast.fire({
            icon: "success",
            html: `
                <div class="text-h6 text-bold text-uppercase">granted!</div>
...
const Enable = async (data) => {
    btnLoading.value = true;
    try {
        const response = await api.post(`/paymentchannel/${data.id}/enable`);
        Toast.fire({
            icon: "success",
            html: `
                <div class="text-h6 text-bold text-uppercase">granted!</div>
